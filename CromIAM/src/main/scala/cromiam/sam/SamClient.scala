...
class SamClient(scheme: String,
  ...
  def isSubmitWhitelistedSam(user: User, cromIamRequest: HttpRequest): FailureResponseOrT[Boolean] = {
    ...
    for {
      ...
      whitelisted <- response.status match {
        case StatusCodes.OK =>
          implicit val whyIsThisNotAutomaticAreThereImportsMissing: Unmarshaller[HttpEntity, Boolean] =
        ...
        case _ => FailureResponseOrT.right[HttpResponse](IO.pure(false))
      }
  ...
  def requestAuth(authorizationRequest: CollectionAuthorizationRequest,
    ...
    def validateEntityBytes(byteString: ByteString): FailureResponseOrT[Unit] = {
      if (byteString.utf8String == "true") {
        FailureResponseOrT.right(IO.unit)
      } else {
        log.warning("Sam denied " + logString)
        FailureResponseOrT.right(IO.raiseError[Unit](new SamDenialException))
      }
    }
  ...
  def requestSubmission(user: User,
    ...
    createCollection flatMap {
      ...
      case r if r.status == StatusCodes.NoContent => FailureResponseOrT.right(IO.unit)
      case r => FailureResponseOrT.right(IO.raiseError[Unit](SamRegisterCollectionException(r.status)))
    }
  }
