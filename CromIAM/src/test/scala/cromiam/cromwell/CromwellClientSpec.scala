...
object CromwellClientSpec {
  ...
  final class MockCromwellApiClient()(implicit actorSystem: ActorSystem, materializer: ActorMaterializer)
    ...
    override def labels(workflowId: WorkflowId, headers: List[HttpHeader] = defaultHeaders)
      ...
      FailureResponseOrT.right(
        if (workflowId == RootWorkflowId) IO.pure(FictitiousWorkflowLabelsWithCollection)
        else if (workflowId == WorkflowIdWithoutCollection) IO.pure(FictitiousWorkflowLabelsWithoutCollection)
        else IO.raiseError(new RuntimeException("Unexpected workflow ID sent to MockCromwellApiClient"))
      )
    }
    ...
    override def metadata(workflowId: WorkflowId,
      ...
      FailureResponseOrT.right(
        if (workflowId == RootWorkflowId) IO.pure(RootWorkflowMetadata)
        else if (workflowId == SubworkflowId) IO.pure(SubWorkflowMetadata)
        else IO.raiseError(new RuntimeException("Unexpected workflow ID sent to MockCromwellApiClient"))
      )
    }
  }
