...
trait CromIamApiService extends RequestSupport
  ...
  FailureResponseOrT[HttpResponse] = {
    def authForCollection(collection: Collection): FailureResponseOrT[Unit] = {
      val samRequest = samClient.requestAuth(CollectionAuthorizationRequest(user, collection, action), request).value
      FailureResponseOrT(
        samRequest handleErrorWith {
    ...
    val cromwellResponseT = for {
      rootWorkflowIds <- workflowIds.map(id => cromwellAuthClient.getRootWorkflow(id, user, request)).sequence
      collections <-
        rootWorkflowIds.map(id => cromwellAuthClient.collectionForWorkflow(id, user, request)).sequence.map(_.distinct)
